...
*******************************************************************************/
void EKF_Init_( AHRS_state_data* estimated_states )
{
	 //int i, j;
	 ...
	 estimated_states->accel_ref_x = 0;
	 estimated_states->accel_ref_y = 0;
	 estimated_states->accel_ref_z = 1;

	 estimated_states->accel_var = 2;
	 estimated_states->process_var = 0.1;
	 estimated_states->mag_var = 2;
...
* Description    : 
*******************************************************************************/
void EKF_EstimateStates( AHRS_state_data* state_data, RawSensorData* sensor_data )
{
...
void ConvertRawSensorData( AHRS_state_data* state_data, RawSensorData* sensor_data )
{
	...
	// svec.rows = 3;
	 ...
	 // Start with rate gyros
         //	 svec.data[0][0] = ((float)(sensor_data->gyro_x - state_data->beta_p));
         //	 svec.data[1][0] = ((float)(sensor_data->gyro_y - state_data->beta_q));
         //	 svec.data[2][0] = ((float)(sensor_data->gyro_z - state_data->beta_r));	 
	 
	 // Copy new gyro data to state_data structure
	 state_data->gyro_x = sensor_data->gyro_x;
	 state_data->gyro_y = sensor_data->gyro_y;
	 state_data->gyro_z = sensor_data->gyro_z;
...
//	 
	 state_data->accel_x = sensor_data->accel_x;
	 state_data->accel_y = sensor_data->accel_y;
	 state_data->accel_z = sensor_data->accel_z;
...
//	 mat_mult( &state_data->mag_cal, &svec, &svec );
	 ...
	 state_data->mag_x = sensor_data->mag_x;
	 state_data->mag_y = sensor_data->mag_y;
	 state_data->mag_z = sensor_data->mag_z;
}
	 ...
	 if( (EKF_MODE_EULER) == 1 )
 	 {
 		  ...
 		  if( 1 )
 		  {
 				static float ax_hat, ay_hat, az_hat;
 				static float ax_ref, ay_ref, az_ref;
...
//	 // QUATERNION CODE
	 else
  	 {
  		 static float a,b,c,d;
  		  ...
  		  if(1)
  		  {
  				float sensor_norm = S_rsqrt(gStateData.accel_x*gStateData.accel_x + gStateData.accel_y*gStateData.accel_y + gStateData.accel_z*gStateData.accel_z);
  				fMatrix C;
                                 ...
                                 //if(0)
  				{
  					static float ax_hat, ay_hat, az_hat;
  					static float ax_ref, ay_ref, az_ref;
  					                             if(*(int*)(&gStateData.qib.a)==0x7FC00000||*(int*)(&gStateData.qib.b)==0x7FC00000||*(int*)(&gStateData.qib.c)==0x7FC00000||*(int*)(&gStateData.qib.d)==0x7FC00000)
          {
            asm("nop");
          }
