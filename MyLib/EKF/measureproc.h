...
typedef struct MV3 { float x[3]; } MV3;
typedef struct MV2 { float x[2]; } MV2;
typedef struct MMAT3 { MV3 x[3]; } MMAT3;




const static float GRAVITATION_ACCELERATION_CONSTANT = 9.80665;
const static float DEFAULT_ACCEL_DISTURBANCE_THRESHOLD_MS2 = 2.941995; //0.3 * GRAVITATION_ACCELERATION_CONSTANT;

const static float DEFAULT_SIGMA_WINDOW_SECONDS = 0.2;

const static float DOUBLE_DEFAULT_ACCELETOMETER_NOISE = 0.0859355543230206;

const static float DEFAULT_GYRO_NOISE = 0.004;
...
const static float FICTIOUS_NOISE_FACTOR = 2.7; // 1.0d;
...
typedef struct kalman {
  ...
  float sigmaWindowLengthSeconds;
