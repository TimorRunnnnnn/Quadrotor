...
short Double_isNaN(float d) {
  return isnan(d);
}

float Math_abs (float d) {
  return fabs(d);
}

float Math_acos (float d) {
  return acos(d);
}

float Math_atan (float d) {
  return atan(d);
}

float Math_cos(float d) {
  return cos(d);
}

float Math_sin(float d) {
  return sin(d);
}

float Math_sqrt(float d) {
  return sqrt(d);
}


int isErrorVector(float * vec, int size) {
  int result = 0;
  int i;
  for (i = 0; i < size; i++) {
...
void updateSigmaValues(KF * pkf,float accelerationNorm, float dt) {
 ...
 * @param vb
 */
void v3AssignVector( MV3*  restrict pva, MV3    vb) {
  pva->x[0] = vb.x[0];
  pva->x[1] = vb.x[1];
  pva->x[2] = vb.x[2];
}
...
void v3DivideScalar(  MV3 inV3vec, float scalar,
                      MV3 * restrict pOutV3Vec) {
 ...
 */
void v3EbeAdd(  MV3 va,   MV3 vb,   MV3 * restrict pvc) {
 ...
 * in - va, vb out - vc vc[i] = va[i]/vb[i]
 */
void v3EbeDivide(  MV3 va,   MV3 vb,   MV3 *  restrict pvc) {
  pvc->x[0] = va.x[0] / vb.x[0];
  pvc->x[1] = va.x[1] / vb.x[1];
  pvc->x[2] = va.x[2] / vb.x[2];
 ...
 * in - va, vb out - vc vc[i] = va[i]*vb[i]
 */
void v3EbeMultiply(  MV3 va,   MV3 vb,   MV3 * restrict pvc) {
  pvc->x[0] = va.x[0] * vb.x[0];
  pvc->x[1] = va.x[1] * vb.x[1];
  pvc->x[2] = va.x[2] * vb.x[2];
 ...
 * in - va, vb out - vc vc[i] = va[i]/vb[i]
 */
void v3EbeSubstract(  MV3 va,   MV3 vb,   MV3 * restrict pvc) {
  pvc->x[0] = va.x[0] - vb.x[0];
  pvc->x[1] = va.x[1] - vb.x[1];
  pvc->x[2] = va.x[2] - vb.x[2];
...
void v3MultiplyScalar(  MV3 inV3vec, float scalar,
                        MV3 * restrict pOutV3Vec) {
...
MMAT3 createKalmanA(KF* pkf,   MV3 angularVelocities,
  ...
  float wx = angularVelocities.x[0];
  float wy = angularVelocities.x[1];
  float wz = angularVelocities.x[2];
  float cf = Math_cos(wkhDtNorm);
  float sf = Math_sin(wkhDtNorm);
...
int currentSigmaValue(KF * pkf) {
  ...
  //return 0;
  int temp=(pkf->idleTimeLength >= pkf->sigmaWindowLengthSeconds) ? 1 : 0;
  return temp;
 ...
 */
void m3MultiplyByv3(   MMAT3 ma,   MV3 v,   MV3 * restrict pOutV) {
...
void updateCovarianceP(KF* pkf,float dt) {
  MV3 pkResult;
...
void updateEstimates(KF* pkf) {
  ...
  if (v3IsZeroNorm(pkf->currentZEstimate)) {
    ...
    float zNorm = v3Norm(pkf->currentZEstimate);
    ...
    /*
      ...
      || float_isNaN(currentXEstimate[2])) {
  ...
  float roll = Math_atan(pkf->currentXEstimate.x[1] / pkf->currentXEstimate.x[2]);
  float pitch = Math_acos(pkf->currentXEstimate.x[2] / Math_cos(roll));
...
void processMeasurement(KF* pkf, float a1, float a2, float a3, float w1,
			float w2, float w3, float dt) {
  ...
  float angularVelocityLimit = (3.14159265358979323846 / 2.0) - 0.0001;
  if (
  ...
  float aNormOriginal = v3Norm(pkf->aVec);
